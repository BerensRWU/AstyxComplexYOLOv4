...
from data_process_astyx.astyx_dataset import astyxDataset
from data_process_astyx.transformation import Compose, OneOf, Random_Rotation, Random_Scaling, Horizontal_Flip, Cutout


def create_train_dataloader(configs):
    """Create dataloader for training"""

    train_point_cloud_transforms = OneOf([
    ...
    train_dataset = astyxDataset(configs.dataset_dir, mode='train', point_cloud_transforms=train_point_cloud_transforms,
                                 aug_transforms=train_aug_transforms, multiscale=configs.multiscale_training, configs=configs,
...
def create_val_dataloader(configs):
    ...
    val_sampler = None
    val_dataset = astyxDataset(configs.dataset_dir, mode='valid', point_cloud_transforms=None, aug_transforms=None,
                               multiscale=False, num_samples=configs.num_samples, configs=configs)
...
def create_test_dataloader(configs):
    ...
    test_dataset = astyxDataset(configs.dataset_dir, mode='test', point_cloud_transforms=None, aug_transforms=None,
                                multiscale=False, num_samples=configs.num_samples, configs=configs)
    test_sampler = None
...
if __name__ == '__main__':
    ...
    import data_process.astyx_bev_utils as bev_utils
    from astyx_data_process import astyx_data_utils
    ...
    configs.dataset_dir = os.path.join('../../../', 'astyx', 'dataset', 'dataset_astyx_hires2019')
    ...
    for batch_i, (img_files, imgs, targets) in enumerate(dataloader):
        if not configs.show_train_data:
            img_file = img_files[0]
            img_rgb = cv2.imread(img_file)
            calib = astyx_data_utils.Calibration(img_file.replace(".png", ".txt").replace("image_2", "calib"))
